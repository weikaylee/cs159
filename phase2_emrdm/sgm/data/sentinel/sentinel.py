...
and returns a batch dict that matches the keys expected by ResidualDiffusionEngine:
    ...
    "target"     (C, H, W) float32        — cloud-free S2 or mirror-space target (input_key)
...
Normalisation:
    S2  (uint16, [0, 10000] reflectance) ÷ 10000  →  [0, 1]  (clipped when clip_target=True)
    S1  (float32, dB, approx [-25, 0])  linear clamp  →  [0, 1]
        # TODO: verify S1 dB range against your archive if results look wrong

    Mirror-space targets (generated by prepare_mirror_dataset.py) are stored as
    y * 10000 in the TIF so that the /10000 rescale round-trips correctly.
    Set clip_target=False when using mirror pkl files — mirror-space values are
    mostly negative (mean ≈ -0.16) and would be destroyed by clipping to [0, 1].
"""
