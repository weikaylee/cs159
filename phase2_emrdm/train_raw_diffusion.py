#!/usr/bin/env python3
"""
Train a standard (non-residual) EDM in raw reflectance space, conditioned on S1S2.

Unlike train_mirror_diffusion.py, this script does NOT require a Phase 1 NAMM
checkpoint. The diffusion model learns to denoise clean S2 reflectance directly,
without any mirror-map transformation.

Training procedure per step
---------------------------
1. Load clean S2 + SAR + cloudy S2 triplet from disk, normalised to [0, 1].
2. Sample sigma ~ log-N(P_mean, P_std^2)
3. Noisy sample: y_t = y + sigma * eps,  eps ~ N(0, I)
4. EDM preconditioning (Karras eq. 7):
      c_skip  = sigma_data^2 / (sigma^2 + sigma_data^2)
      c_out   = sigma * sigma_data / sqrt(sigma^2 + sigma_data^2)
      c_in    = 1 / sqrt(sigma^2 + sigma_data^2)
      c_noise = 0.25 * log(sigma)
5. Network input: [c_in * y_t  ||  s1s2]  →  F_theta  →  raw_out
   Denoised prediction: D = c_skip * y_t + c_out * raw_out
6. Loss: w(sigma) * mean(||D - y||^2),   w = (sigma^2 + sigma_data^2) / (sigma * sigma_data)^2

Inference (not implemented here)
---------------------------------
Start from x_T ~ N(0, sigma_max^2 * I) in reflectance space.
Run an EDM sampler (e.g. Euler) conditioned on SAR + cloudy S2.
Predictions are already in reflectance space — no inverse map needed.

Usage
-----
    cd phase2_emrdm
    python train_raw_diffusion.py \\
        --data_root /resnick/groups/CS156b/from_central/data/train \\
        --output_dir /resnick/groups/perona/oywang/cs159/runs/raw_edm \\
...
class SEN12MSCRTripletDataset(Dataset):
    """Load SEN12MS-CR triplets for raw-reflectance EDM training.
    ...
    Returns (per item):
        s2_clean: (13, H, W) float32 in [0, 1]   — diffusion target
    ...
    def __getitem__(self, idx: int):
        ...
        return torch.tensor(np.array(s2), dtype=torch.float32), torch.tensor(np.array(s1s2), dtype=torch.float32)
...
class EDMUNet(nn.Module):
    ...
    Input channels  = noisy S2 reflectance (out_ch=13) + S1S2 conditioning (15) = 28.
    Output channels = S2 reflectance (13).
    Sigma is injected through a Fourier embedding → MLP → AdaGN at every block.

    Args:
        in_ch:   total input channels (noisy S2 + S1S2). Default 28.
        out_ch:  output channels (reflectance). Default 13.
...
class EDMDenoiser(nn.Module):
    ...
    raw S2 reflectance target distribution (values in [0, 1]). A reasonable
    default is 0.5; measure the actual std from your data to tune further.
    """

    def __init__(self, network: EDMUNet, sigma_data: float = 0.091):
        super().__init__()
    ...
    def forward(
        self,
        y_noisy: torch.Tensor,  # (B, 13, H, W) noisy reflectance target
...
def edm_loss(
    ...
    y:    torch.Tensor,   # (B, 13, H, W) clean reflectance target
...
def val_metrics(
    ...
    Since the model operates directly in reflectance space, benchmark metrics
    (mae, sam, psnr, ssim) are computed by comparing the single-step denoised
    prediction against the ground-truth clean S2.
    ...
    y_noisy = y + sigma[:, None, None, None] * torch.randn_like(y)
    pred    = denoiser(y_noisy, sigma, s1s2).clamp(0.0, 1.0)

    m = reconstruction_metrics(pred, y)
    return {
        "mae":  m["mae"].item(),
        "sam":  m["sam"].item(),
        "psnr": m["psnr"].item(),
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Standard EDM in raw reflectance space, conditioned on S1S2."
    )
    # Paths
    ...
    # EDM hyperparameters
    p.add_argument("--sigma_data", type=float, default=0.091,
                   help="~std of raw S2 reflectance targets in [0,1]. "
                        "Measured at 0.091 (global std over all channels/patches).")
...
def main():
    ...
    if args.wandb:
        ...
        wandb.init(
            ...
            name=args.wandb_run_name or "raw-edm",
            config=vars(args),
        )
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    network  = EDMUNet(
        in_ch   = args.n_channels + 15,   # 13 reflectance + 15 S1S2
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (s2_clean, s1s2) in enumerate(train_loader):
            ...
            with autocast(enabled=args.fp16):
                loss = edm_loss(denoiser, s2_clean, s1s2,
            ...
            if (step + 1) % args.log_every == 0:
                ...
                denoiser.eval()
                tm = val_metrics(denoiser, s2_clean, s1s2)
                denoiser.train()

                print(
                    f"  [ep {epoch:03d} step {step + 1:05d}]"
                    f"  loss={mean_loss:.4f}"
                    f"  mae={tm['mae']:.4f}  sam={tm['sam']:.4f}"
                    f"  psnr={tm['psnr']:.2f}  ssim={tm['ssim']:.4f}"
                ...
                with open(log_path, "a", newline="") as f:
                    csv.DictWriter(f, fieldnames=csv_fields).writerow(dict(
                        ...
                        mae=f"{tm['mae']:.6f}", sam=f"{tm['sam']:.6f}",
                        psnr=f"{tm['psnr']:.4f}", ssim=f"{tm['ssim']:.6f}",
                    ))
                if args.wandb:
                    import wandb
                    wandb.log({
        ...
        with torch.no_grad():
            for s2_clean, s1s2 in val_loader:
                ...
                v_losses.append(
                    edm_loss(denoiser, s2_clean, s1s2,
                             args.P_mean, args.P_std, args.s2_dropout).item()
                )
                m = val_metrics(denoiser, s2_clean, s1s2)
                v_mae.append(m["mae"])
                v_sam.append(m["sam"])
        ...
        val_mae  = float(np.mean(v_mae))
        val_sam  = float(np.mean(v_sam))
        val_psnr = float(np.mean(v_psnr))
        val_ssim = float(np.mean(v_ssim))
        elapsed  = time.time() - t0
