...
class ImageLogger(Callback):
    def __init__(
        ...
        assert scale != clamp, "clamp and scale cannot be the same"
        self.scale = scale
    ...
    def log_local(
        ...
        for k in images:
            if isheatmap(images[k]):
                ...
                if self.rescale and not self.scale:
        ...
        if (
            ...
            for k in images:
                ...
                if isinstance(images[k], torch.Tensor):
                    ...
                    if self.scale and not isheatmap(images[k]):
                        images[k] =  (images[k] - images[k].min()) / (images[k].max() - images[k].min())
...
if __name__ == "__main__":
    ...
    f.manual_seed(opt.seed + 0)  # note:  this may get re-seeded each epoch
    g.manual_seed(opt.seed)      #        keep this one fixed
    ...
    try:
        ...
        print(f"Model has {num_params:.4f}M parameters.")
        ...
        if "profiler" in lightning_config:
            trainer_kwargs['profiler'] =  lightning_config.profiler if isinstance(lightning_config.profiler, str) else instantiate_from_config(lightning_config.profiler)
        ...
        if not cpu:
            _devices = lightning_config.trainer.devices
            ngpu = _devices if isinstance(_devices, int) else len(str(_devices).strip(",").split(","))
        else:
            ngpu = 1
