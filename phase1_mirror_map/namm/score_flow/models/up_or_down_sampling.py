...
def upfirdn_3d(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
  ...
  inW = x.shape[3]
  minorDim = x.shape[4]
  kernelD, kernelH, kernelW = k.shape
  assert inW >= 1 and inH >= 1
  assert kernelW >= 1 and kernelH >= 1
  assert isinstance(upx, int) and isinstance(upy, int)
  assert isinstance(downx, int) and isinstance(downy, int)
...
def _simple_upfirdn_3d(x, k, up=1, down=1, pad0=0, pad1=0, data_format='NDHWC'):
...
def upsample_3d(x, k=None, factor=2, gain=1, data_format='NHWC'):
  assert isinstance(factor, int) and factor >= 1
  if k is None:
    k = [1] * factor
  k = _setup_kernel_3d(k) * (gain * (factor ** 2))
  p = k.shape[0] - factor
  return _simple_upfirdn_2d(
    x,
    k,
