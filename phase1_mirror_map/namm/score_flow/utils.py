...
def normalize_array(imarr, vmin=None, vmax=None, normtype='sigmoid'):
  if vmin is None:
    vmin = jnp.min(imarr)
  if vmax is None:
    vmax = jnp.max(imarr)
  if normtype == 'sigmoid':
    ...
    return (imarr - vmin) / ((vmax - vmin) / 2) - 1
  else:
    raise ValueError(f'Unsupported normalization: {normtype}')


def get_latest_step(ckpt_dir):
  """Get step of latest checkpoint in `ckpt_dir`."""
  paths = glob(f'{ckpt_dir}/checkpoint_*')
  latest_step = 0
  for path in paths:
...
def visualize_weight_fn(weight_fn, n_steps=20000, title='weight'):
  """See weight annealing function plotted over `n_steps`."""
  steps = np.arange(n_steps)
  weights = [weight_fn(s) for s in steps]
...
def get_stepsize_controller(stepsize_controller,
                             rtol,
                             atol,
