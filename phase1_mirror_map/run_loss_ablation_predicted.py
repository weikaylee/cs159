"""
Phase 1 loss-ablation sweep using EMRDM-predicted images as paired input.

Identical to run_loss_ablation.py except that training uses
SEN12MSCREMRDMOutputDataset: ground truth is data/{roi}_s2 and the paired
"noisy" input is the EMRDM-predicted cloud-removed images from --emrdm_root
(default: ./results).  Pass --emrdm_root to point at the folder produced by
model.py, which has the structure:

    results/
        ROIs1158_spring_s2/   *.tif
        ROIs1868_summer_s2/   *.tif
        ROIs1970_fall_s2/     *.tif
        ROIs2017_winter_s2/   *.tif

Filenames in emrdm_root are matched to ground-truth patches by basename, so
the EMRDM output files must share the same name as the cloud-free GT patches.

Note: dataset.py globs for *.tif in emrdm_root. If EMRDM wrote PNGs, convert
them to GeoTIFF first before running this sweep.

Usage:
    # inspect the configs (no training)
    python run_loss_ablation_predicted.py --list
    python run_loss_ablation_predicted.py --data_root <DATA> --dry_run

    # run on local test data
    python phase1_mirror_map/run_loss_ablation_predicted.py \
      --data_root ./data \
      --emrdm_root ./results \
      --output_root runs/loss_ablation_predicted \
      --roi all \
      --epochs 1 --batch_size 8 --patch_size 64 --num_workers 0 \
      --wandb --wandb_project tests

    # run the whole sweep sequentially
    python run_loss_ablation_predicted.py \
      --data_root <DATA> --emrdm_root <RESULTS> \
      --output_root <OUT> --fp16 --wandb

    # run a single config (SLURM array task)
    python run_loss_ablation_predicted.py \
      --data_root <DATA> --emrdm_root <RESULTS> --output_root <OUT> \
      --config_index $SLURM_ARRAY_TASK_ID --fp16 --wandb

    # rebuild ablation_summary.csv from finished runs
    python run_loss_ablation_predicted.py --output_root <OUT> --collate_only
...
def get_args():
    p = argparse.ArgumentParser(
        description='Phase 1 loss-ablation sweep (EMRDM-predicted inputs)',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    # Sweep / data
    p.add_argument('--data_root', default=None,
                   help='SEN12MS-CR data root with cloud-free *_s2 folders '
                        '(required unless --list/--collate_only)')
    p.add_argument('--emrdm_root', default=os.path.join(os.getcwd(), 'results'),
                   help='Root of EMRDM-predicted outputs (*.tif matched by basename '
                        'to cloud-free GT patches)')
    p.add_argument('--output_root', default=os.path.join(os.getcwd(),
                                                         'runs', 'loss_ablation_predicted'),
    ...
    p.add_argument('--wandb_prefix', default='ablation-pred-',
...
def build_command(cfg: dict, args) -> list[str]:
    ...
    cmd = [
        ...
        '--emrdm_loss_weight', str(args.emrdm_loss_weight),
        '--output_dir', out_dir,
        '--roi', args.roi,
...
def log_ablation_to_wandb(rows: list[dict], output_root: str, args) -> None:
    ...
    if os.path.isfile(summary_path):
        artifact = wandb.Artifact('ablation_summary_predicted', type='dataset')
...
def main() -> None:
    ...
    args.emrdm_root = os.path.abspath(args.emrdm_root)
    ...
    if failures:
        print(f'\n{len(failures)} config(s) failed: {", ".join(failures)}',
              file=sys.stderr)
        sys.exit(1)
    print('\nLoss-ablation sweep (predicted inputs) complete.')
